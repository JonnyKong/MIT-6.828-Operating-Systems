...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	// lookup system open file table to get offset
	struct OpenFile *o;
	int r;
	if ((r = openfile_lookup(envid, req->req_fileid, &o)) < 0)
		return r;

	// read the file into the response buffer
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	// lookup system open file table to get seek pos
	struct OpenFile *o;
	int r;
	
	if ((r = openfile_lookup(envid, req->req_fileid, &o)) < 0)
		return r;
	cprintf("will write some bytes, first int is %d\n",*(int *)(req->req_buf));
